...
export const log = {
   ...
   *
   * A wall-clock number in the log is the only way to tell a slow repository
   * from a hung one after the fact, on a machine nobody here can reach. A stack
   * trace taken after a timeout says which command was cut off but not whether
   * it was about to finish, so it cannot separate "this disk is slow" from
   * "this repository never answers" - the two that need different fixes.
   */
